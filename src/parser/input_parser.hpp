...
private:
    void parseOptions(int argc, char* argv[]) {
        for (int i = 1; i < argc; i++) {
            ...
            } else if (strcmp(argv[i], "-n") == 0 || strcmp(argv[i], "--input-name") == 0) {
                i++;
                inputData_.fileName = argv[i];
            } else if (strcmp(argv[i], "-t") == 0 || strcmp(argv[i], "--input-type") == 0) {
                i++;
                inputData_.fileType = argv[i];
            } else if (strcmp(argv[i], "-o") == 0 || strcmp(argv[i], "--output-path") == 0) {
                i++;
                outputData_.outputFilePath = argv[i];
            } else if (strcmp(argv[i], "-s") == 0 || strcmp(argv[i], "--output-name") == 0) {
                i++;
                outputData_.outputFileName = argv[i];
            } else if (strcmp(argv[i], "-e") == 0 || strcmp(argv[i], "--output-type") == 0) {
                i++;
    ...
    void displayHelp(int argc, char* argv[]) {
        ...
        std::cout << "  -t, --input-type   Input file type (video, image, pointcloud)" << std::endl;
        std::cout << "  -o, --output-path  Output file path" << std::endl;
        std::cout << "  -s, --output-name  Output file name" << std::endl;
        std::cout << "  -e, --output-type  Output file type (video, image, pointcloud)" << std::endl;
    }
